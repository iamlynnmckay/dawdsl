...
function self._call_on_number_or_array(f, xs)
    if dawdsl.std.type.is_array(xs) then
        ...
        assert(dawdsl.std.type.is_number(xs))
        return f(xs)
    end
end
...
function self.floor(x)
    assert(dawdsl.std.type.is_number(x) or dawdsl.std.type.is_array(x))
...
function self.normalize(x, y_max, y_min, x_max, x_min)
    ...
    assert(dawdsl.std.type.is_number(x) or dawdsl.std.type.is_array(x))
    assert(dawdsl.std.type.is_number(y_max) or dawdsl.std.type.is_array(y_max))
    assert(dawdsl.std.type.is_number(y_min) or dawdsl.std.type.is_array(y_min))
    assert(dawdsl.std.type.is_number(x_max) or dawdsl.std.type.is_array(x_max))
    assert(dawdsl.std.type.is_number(x_min) or dawdsl.std.type.is_array(x_min))
    assert(y_min < y_max)
    assert(x_min < y_max)
